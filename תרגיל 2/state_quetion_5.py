...
def hdistance(s):  # the heuristic value of s
    ...
    for i in range(len(s[0])):
        if not i == s[0][i]:
            ...
            x = i % math.sqrt(len(s[0]))
            y = i / math.sqrt(len(s[0]))

            for j in range(len(s[0])):
                if s[0][j] == i :
                    index = j

            index_x = index % math.sqrt(len(s[0]))
            index_y = index / math.sqrt(len(s[0]))

            distance = distance + abs(index_x-x)
            distance = distance + abs(index_y-y)
